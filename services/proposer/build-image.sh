...
#      below needs it bind-mounted TWICE (see the Dockerfile's own header comment for why:
#      av-cdm's spoore-cdm dependency is a relative sibling path since question 219(c), but
#      av-proposer's OWN spoore-models path dependency and its build.rs's direct read of
#      spoore's model_service.proto are still absolute `/Users/probe/code/spoore/...` paths
#      -- the heavy team's remaining half of question 219(b)/(c), out of this track's scope
#      (crates/av-proposer is off-limits here) -- so this script mounts the same SPOORE_ROOT
#      at both the fixed absolute path av-proposer's manifest/build.rs still expect AND the
#      sibling path spoore-cdm's relative dependency now resolves to).
...
#      repository AND SPOORE_ROOT TWICE, read-only (at the sibling of the repository's own
#      mount point for spoore-cdm's relative resolution, and at the fixed absolute path
#      av-proposer's own still-absolute dependencies expect -- question 219(b)/(c)) into a
...
# Two container mount destinations are needed here, unlike services/edge-plugin/
# build-image.sh, computed in this one place rather than hardcoded twice:
#   - SPOORE_CONTAINER_PATH: the sibling of CONTAINER_WORKSPACE, which is what spoore-cdm's
#     now-relative workspace dependency (`../spoore/crates/spoore-cdm`, question 219(c))
#     resolves to inside the container, exactly as services/edge-plugin/build-image.sh's own
#     mount does.
#   - SPOORE_FIXED_ABS_PATH: crates/av-proposer/Cargo.toml's OWN spoore-models path
#     dependency, and its build.rs's own direct read of spoore's model_service.proto, are
#     still literal absolute `/Users/probe/code/spoore/...` strings (av-proposer is
#     off-limits to this track -- the heavy team's remaining half of question 219(b)/(c)) --
#     so SPOORE_ROOT (wherever it actually is on THIS host) is ALSO mounted at that exact
#     fixed path inside the container, independent of where SPOORE_ROOT itself lives, since a
#     bind mount's host source and container destination are chosen independently.
CONTAINER_WORKSPACE="/workspace"
SPOORE_CONTAINER_PATH="$(dirname "${CONTAINER_WORKSPACE}")/spoore"
SPOORE_FIXED_ABS_PATH="/Users/probe/code/spoore"
...
log "prebuilding av-proposer (release, stripped) via ${PREBUILD_BASE_IMAGE} with ${SPOORE_ROOT} bind-mounted read-only at BOTH ${SPOORE_CONTAINER_PATH} (the sibling of ${CONTAINER_WORKSPACE}, question 219(b)/(c): what av-cdm's now-relative spoore-cdm dependency resolves to) AND ${SPOORE_FIXED_ABS_PATH} (what av-proposer's own still-absolute spoore-models dependency and its build.rs's proto read still expect -- the heavy team's remaining half of question 219(b)/(c), crates/av-proposer being off-limits here) -- this is the one permitted network window (question 154): apt packages inside the prebuild container"
docker run --rm \
    -v "${REPO_ROOT}:${CONTAINER_WORKSPACE}" \
    -v "${SPOORE_ROOT}:${SPOORE_CONTAINER_PATH}:ro" \
    -v "${SPOORE_ROOT}:${SPOORE_FIXED_ABS_PATH}:ro" \
