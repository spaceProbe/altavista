...
| ID | Requirement | Status | Implementation | Evidence |
|---|---|---|---|---|
| 3.5.1 / 3.5.2 | Identify and authenticate users/processes | Partial | **Question 207, closed this round**: `av-edge-plugin`'s own binary now calls `crate::identity::verify_identity`/`TrustAnchors` (`crates/av-edge/src/identity.rs`) directly. Given `--client-cert`/`--trust-anchor` (an argument error if only one is given — `parse_args`), `run`/`verify_own_identity` verify this process's own leaf against the configured Root, at an injected clock (`--now-tai-ns`, or the real clock when absent), **before dialing `--endpoint`, announcing, or signing a single batch**: a rejection prints the typed `IdentityRejection` and `IdentityCounters` to stderr and exits non-zero with the ingest never touched (`crates/av-ingest-client/tests/av_edge_plugin_identity.rs::the_plugin_refuses_its_own_untrusted_leaf_before_connecting` proves the ingest's own counters stay entirely zero). On success the verified fingerprint overrides `PluginConfig.leaf_fingerprint_sha256` (so the manifest and every batch's `signer_cert_sha256` cannot drift from the certificate actually checked) and is forwarded on the wire (`x-ssl-client-escaped-cert`, plaintext path only) so `av-ingest` re-verifies it independently and counts the result — `av_edge_plugin_identity.rs::a_leaf_from_a_wrong_ca_reaches_the_wire_and_is_refused_and_counted_end_to_end` is this round's own end-to-end proof that a leaf from an unrelated CA is refused at `Announce` and `IdentityCounters.issuer_not_trusted` increments by exactly one, read back through the ingest's own evidence surface, with zero accepted batches and no batch ever reaching a partition log. **Still Partial, not Met**: `--client-cert`/`--trust-anchor` are optional flags — a plugin run without them is exactly as identity-blind as before this round, and this row's own SC 3.13.8 sibling (below) still applies unchanged: no test in this repository proves this binary completing a real mTLS handshake, so the `https://` front's own encryption is still unproven for this component | `cargo test -p av-ingest-client --test av_edge_plugin_identity -- --nocapture` (four tests: a good leaf accepted end to end; a wrong-CA leaf reaching the wire and refused with the counter proof above; the plugin refusing its own untrusted leaf before ever connecting; certificate expiry checked against an injected, never-slept clock) and `grep -n "identity" crates/av-ingest-client/src/bin/av-edge-plugin.rs` (now returns real hits — `av_edge::identity::{IdentityCounters, IdentityRejection, TrustAnchors}`, `verify_own_identity`, `verify_identity`, and this file's own module-doc section named "Presenting and verifying this plugin's own identity") |
...
3. **Closed this round (question 207) — was: no caller/process identity check anywhere on
   the plugin's own call path (IA 3.5.1/3.5.2)**. `av-edge-plugin`'s own binary now calls
   `crate::identity::verify_identity`/`TrustAnchors` directly, given `--client-cert`/
   `--trust-anchor` (`crates/av-ingest-client/tests/av_edge_plugin_identity.rs`, four
   tests): a leaf that does not chain to the plugin's own configured Root is refused and
   printed before the binary ever dials `--endpoint`, and a leaf that does verify locally
   is still re-verified by `av-ingest` itself once forwarded on the wire (the wrong-CA case
   is refused there too, with `IdentityCounters.issuer_not_trusted` incrementing by
   exactly one, read back through the ingest's own evidence surface). **Still a real,
   narrower gap, not fully closed**: `--client-cert`/`--trust-anchor` are optional — a
   deployment that omits them is exactly as identity-blind as before this round, and this
   round adds no enforcement making them mandatory (that would be a deployment/operator
   policy, e.g. a wrapper script or container entrypoint requiring the flags, not something
   this binary's own `parse_args` can decide on its own). The mTLS front itself (deficiency
   2, SC 3.13.8) remains unproven for this binary either way.
