...
| 3.3.2 | Trace actions to individual users/processes | Met | **R5.1**: `ProposalEvidence.model_identity` (`crates/av-gateway/src/propose_flow.rs::authenticated_propose_command`) now records the VERIFIED service-token subject — never the caller-declared `principal`, which is refused outright if non-empty and disagreeing (`crate::auth::AuthRefusal::PrincipalMismatch`, mirroring `av-command`'s own `AckRequest.principal` convention) — so this field traces to a real, cryptographically-verified identity, not a caller-supplied string. D5's `query_ids` still trace which specific queries a model session saw before proposing, matched against what a real `GatewayCore::query` call actually served. **R5.1b, defect 2**: R5.1 traced WHO submitted a proposal but stopped naming WHICH MODEL did — `ProposalEvidence.model_node_id` (additive, `proto/altavista/v1/authority.proto`) restores that half. A real evidence record now carries all three, and the three are NOT equally trustworthy: `model_identity` is VERIFIED (the OIDC token subject `av_command::oidc::verify` produced — trace-worthy on its own), while `model_node_id` and `model_version` are caller-DECLARED and UNVERIFIED (the proposer's own `--model-node-id`/`--model-version` values, never checked against any token claim — useful for attribution, not for accountability). A reviewer tracing "who did this" reads `model_identity`; a reviewer asking "which model implementation" reads `model_node_id`, on trust | `cargo test -p av-gateway --lib auth::tests::a_service_token_proposes_and_the_verified_subject_is_returned propose_flow::tests::authenticated::a_disagreeing_declared_principal_is_refused_and_counted` and `cargo test -p av-gateway --test propose_flow_agreement two_propose_surfaces_agree_on_the_same_outcome_for_equivalent_input` (both surfaces' evidence records the identical verified subject) and `cargo test -p av-gateway --test propose_only evidence_round_trips_the_run_identity_and_query_ids_the_gateway_actually_served` and (R5.1b) `cargo test -p av-proposer --test model_service model_infos_identity_is_exactly_what_a_real_proposals_evidence_carries` — run for real, passing: a real proposal's evidence read back off the real ledger carries `model_identity` = the verified service token subject, `model_node_id` = the declared `ModelIdentity.node_id`, and `model_version` = the served `ModelInfo.version`, all three simultaneously |
