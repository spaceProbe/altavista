...
fn a_command_reapplied_after_a_fault_rematerialization_emits_a_second_event_even_though_its_value_never_changed() {
    ...
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-port-cmd-rematerialize".to_string(), error_mode: Default::default() , products_dir: None, replay: None };
