...
fn two_runs_of_the_same_drm_produce_byte_identical_trajectories() {
    ...
    let run = || {
        let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-determinism".to_string(), error_mode: Default::default() , products_dir: None, replay: None };
...
fn a_freshly_rebuilt_but_content_identical_drm_produces_the_same_bytes() {
    ...
    let cfg_a = RunConfig { gmat: &gmat, drm: &drm_a, sos: &sos_a, systems: &systems_a, run_id: "test-run-determinism-fresh".to_string(), error_mode: Default::default() , products_dir: None, replay: None };
    let mut products_a = execute(cfg_a).expect("DRM executes end to end");
    let a = products_a.trajectories.remove("veh").unwrap();

    let cfg_b = RunConfig { gmat: &gmat, drm: &drm_b, sos: &sos_b, systems: &systems_b, run_id: "test-run-determinism-fresh".to_string(), error_mode: Default::default() , products_dir: None, replay: None };
