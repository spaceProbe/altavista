...
fn drm_matches_the_golden_arc() {
    ...
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-drm-golden-1".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
...
fn drm_rmag_output_matches_a_genuine_gmat_reportfile() {
    ...
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &rmag_drm, sos: &sos, systems: &systems_map, run_id: "test-run-rmag".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let products = execute(cfg).expect("DRM executes end to end");
...
fn drm_covariance_matches_the_golden_stm_and_propagated_cov() {
    ...
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-drm-covariance".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
...
fn covariance_at_a_coarser_step_rate_than_the_sample_interval_succeeds_end_to_end() {
    ...
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-coarse-cov".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
...
fn covariance_requested_without_initial_covariance_is_refused() {
    ...
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-missing-p0".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
...
fn a_non_spd_initial_covariance_is_refused_with_the_typed_hygiene_error_and_counted() {
    ...
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-bad-p0".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let err = execute(cfg).unwrap_err();
...
fn a_tampered_drm_hash_is_refused() {
    ...
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-tampered".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let err = execute(cfg).unwrap_err();
...
fn a_drm_whose_content_no_longer_matches_its_declared_hash_is_refused() {
    ...
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-content-tampered".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let err = execute(cfg).unwrap_err();
    assert!(matches!(err, DrmError::HashMismatch { artifact: "DesignReferenceMission", .. }), "{err:?}");
}
...
fn covariance_with_relativistic_correction_is_refused_unless_accepted() {
    ...
    systems.insert(sys.id.clone(), sys);
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-rc".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let err = execute(cfg).unwrap_err();
...
fn running_the_identical_drm_twice_in_one_process_produces_byte_identical_products() {
    ...
    let cfg1 = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: run_id.clone(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let products1 = execute(cfg1).expect("the first execute() call succeeds");
    let cfg2 = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: run_id.clone(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
...
fn a_renode_binding_is_refused_through_the_full_executor() {
    ...
    systems.insert(sys.id.clone(), sys);
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-renode".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let err = execute(cfg).unwrap_err();
...
fn a_connection_to_an_undeclared_port_is_refused_through_the_full_executor() {
    ...
    systems.insert(sys.id.clone(), sys);
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-router".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let err = execute(cfg).unwrap_err();
...
fn a_dynamics_fault_splits_the_run_into_two_segments_with_continuous_state() {
    ...
    systems.insert(sys.id.clone(), sys);
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-accel-fault".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
...
fn output_speed_resolves_against_a_real_gmat_bound_instance() {
    ...
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &drm, sos: &sos, systems: &systems, run_id: "test-run-output-speed".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let products = execute(cfg).expect("DRM executes end to end");
...
fn covariance_and_plain_paths_produce_the_same_named_output_set() {
    ...
    let cfg_plain = RunConfig { gmat: &gmat, drm: &drm_plain, sos: &sos_plain, systems: &systems_map, run_id: "test-run-outputs-plain".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    ...
    let cfg_cov = RunConfig { gmat: &gmat, drm: &drm_cov, sos: &sos_cov, systems: &systems_map, run_id: "test-run-outputs-cov".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
