...
fn run_probe(coordinate_system: &str, instance_name: &str, gmat: &Gmat) -> RunProducts {
    ...
    let cfg = RunConfig { gmat, drm: &drm, sos: &sos, systems: &systems, run_id: format!("test-run-covariance-frame-conversion-{instance_name}"), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
