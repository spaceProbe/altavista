...
fn straight_accel_objectives_and_moes_match_the_closed_form_solution() {
    ...
    let cfg = RunConfig { gmat: &gmat, drm: &case.drm, sos: &case.sos, systems: &case.systems, run_id: "test-expr-straight-accel".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
...
fn fault_split_accel_objectives_resolve_at_time_by_event_name() {
    ...
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &case.drm, sos: &case.sos, systems: &case.systems, run_id: "test-expr-fault-split".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let products = execute(cfg).expect("DRM executes end to end");
...
fn an_unknown_entity_in_a_declared_objective_is_refused_at_load() {
    ...
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &case.drm, sos: &case.sos, systems: &case.systems, run_id: "test-expr-unknown-entity".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
...
fn an_unknown_entity_against_a_real_run_is_a_typed_expr_error() {
    ...
    let case = straight_accel_case();
    let gmat = Gmat::setup(&Gmat::default_startup_file()).expect("GMAT setup");
    let cfg = RunConfig { gmat: &gmat, drm: &case.drm, sos: &case.sos, systems: &case.systems, run_id: "test-expr-unknown-entity-2".to_string(), error_mode: Default::default() , products_dir: None, replay: None, command_source: None };
    let products = execute(cfg).expect("DRM executes end to end");
    let scenario = case.drm.scenario.as_ref().unwrap();
