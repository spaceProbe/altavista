...
fn ground_issued_telecommand_applies_at_the_same_epoch_and_produces_a_byte_identical_arc() {
    ...
    let cfg_signal = RunConfig { gmat: &gmat, drm: &drm_signal, sos: &sos_signal, systems: &systems_signal, run_id: "test-ground-cmd-signal-reference".to_string(), error_mode: Default::default() };
    ...
    let cfg_ground = RunConfig { gmat: &gmat, drm: &drm_ground, sos: &sos_ground, systems: &systems_ground, run_id: "test-ground-cmd-ground-issued".to_string(), error_mode: Default::default() };
