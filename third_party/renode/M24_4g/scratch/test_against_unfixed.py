...
# DIAGNOSTIC-ONLY COPY (third_party/renode/M24_4g/scratch/): FIXED_BRIDGE_PATH deliberately
# repointed at the frozen pre-fix snapshot, to PROVE this test suite fails against the unfixed
# implementation before the fix was applied to the real renode_bridge.py. Not part of the
# permanent suite (outside tests/, so pytest's own testpaths never collect it).
FIXED_BRIDGE_PATH = REPO_ROOT / "third_party" / "renode" / "M24_4g" / "renode_bridge_UNFIXED_snapshot.py"
...
class ScriptedSocket:
    ...
    `chunks` is a list of byte strings. Each one becomes visible the next time something calls a
    *blocking* `recv()` while nothing is currently pending -- modelling "the peer's next scheduled
    bytes arrive now", i.e. real transmission order, without needing any real timing or sleeping.

    A non-blocking `MSG_PEEK` (what the M24.4g fix's stray-byte check uses) only ever reports what
    is ALREADY pending; it never advances the script. This matters for fidelity: in the real
    protocol the peer only sends a reply after receiving the request that provoked it, so a peek
    taken before this client has even sent its own next command must not see a future command's
    reply materialize early -- matching a real non-blocking `recv()`, which raises rather than
    returning data (or b"") when nothing is actually queued yet.
    """
...
def test_desync_reproduces_against_frozen_unfixed_snapshot():
    ...
    sock = ScriptedSocket([
        f'{PROMPT} emulation RunFor "0.050000"'.encode(),
        TIMEOUT,
        CAPTURED_LEFTOVER_TEXT.encode(),
    ])
...
def test_fixed_monitor_client_returns_the_real_register_value():
    ...
    # Three chunks, delivered in order as each blocking recv() needs more:
    ...
    #   C) ReadDoubleWord's own real reply, with the real register value.
    sock = ScriptedSocket([
        f'{PROMPT} emulation RunFor "0.050000"'.encode(),
        f'\r\n{PROMPT} '.encode(),
...
def test_fixed_monitor_client_raises_typed_desync_error_on_stray_bytes():
    ...
    sock = ScriptedSocket([CAPTURED_LEFTOVER_TEXT.encode()])
    # Pre-seed `_pending` directly so the very first stray-byte peek (before ANY command is sent)
    # already finds it queued -- bypassing the "only reveal chunks on a blocking recv" modelling
    # convenience above, which does not apply to bytes that arrived before this client even
    # started (e.g. an earlier `cmd()` call elsewhere that itself failed to fully drain).
    sock._pending.extend(sock._chunks.pop(0))
