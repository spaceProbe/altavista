...
def main():
    ...
    monitor_port = 15023
    bridge_port = 15024
    uart_log = "/Users/probe/code/AltaVista/third_party/renode/M24_4b/uart1_register_probe_v2_uart0.log"
    log_path = "/Users/probe/code/AltaVista/third_party/renode/M24_4b/uart1_register_probe_v2.renode_log.txt"
    ...
    with open(log_path, "wb") as logf:
        ...
        try:
            ...
            # VARIANT (v2): advance the emulation by 1 virtual second BEFORE the TCP peer ever
            # connects, unlike the original probe (and unlike cfs_handshake_test.py) which
            # connects the peer before any RunFor at all. Tests whether Renode's terminal needs
            # the emulation to have already been stepped at least once for a freshly-accepted
            # TCP client's byte stream to be wired into the peripheral's receive path.
            client.send('emulation RunFor "0.3"')
            ...
            print(f"BASELINE (post 0.3s RunFor, pre-connect) control=0x{ctrl0:08x} channel_sts=0x{sts0:08x}")
            samples.append({"phase": "pre-connect-post-1s-run", "control": ctrl0, "channel_sts": sts0})

            bridge_client = socket.create_connection(("127.0.0.1", bridge_port), timeout=5.0)
            print("bridge client connected (AFTER 1s of RunFor)")
    ...
    with open("/Users/probe/code/AltaVista/third_party/renode/M24_4b/uart1_register_probe_v2_result.json", "w") as f:
